...
function StroopTest() {
  ...
  const [currentCurrency, setCurrentCurrency] = useState({ symbol: '', name: '', country: '' });
    ...
    <div className="stroop-test">
      {showInstructions ? (
        <div className="instructions">
          <h1>Instructions</h1>
          <ul>
            <li>Select the correct country for the displayed currency symbol.</li>
            <li>There will be 8 rounds. Try to score as high as possible.</li>
          </ul>
          <button className="high" onClick={() => { setShowInstructions(false); setStart(true); }}>Start Test</button>
        </div>
      ) : start ? (
        <div className='test-div'>
          ...
          <div className='btns'>
            {currencies.map((currency, index) => (
              <button className='next' key={index} onClick={() => handleOptionClick(currency.country)}>
                {currency.country}
              </button>
            ))}
          </div>
          {round === 8 && (
            <div className='btn-test'>
              ...
              <div>
                <button className='start' onClick={startOver}>Start Over</button>
        ...
        <div className='background'>
          <button className="high" onClick={() => setShowInstructions(true)}>Start Test</button>
        </div>
      )}
    </div>
  );
